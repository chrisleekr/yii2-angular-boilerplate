...
export class SettingGlobalFormComponent implements OnInit, OnDestroy {
  ...
  resetSetting() {
    ...
    this.setting.meta_attribute = '{"list":[{"value":"SAMPLE VALUE 1","label":"SAMPLE LABEL 1"},{"value":"SAMPLE VALUE 2","label":"SAMPLE LABEL 2"}]}';
    ...
    _.forIn(this.form.getRawValue(), (value, key) => {
      if (typeof this.setting[key] !== 'undefined') {
        if (value instanceof moment) {
          this.setting[key] = moment(value, environment.customDateTimeFormat.parseInput, true).format(environment.customDateTimeFormat.apiFormat);
        } else {
  ...
  public ngOnInit() {
    ...
    this.parameters = this.activatedRoute.params.subscribe(params => {
      ...
      if (typeof params['id'] !== 'undefined') {
        ...
        this.errorMessage = '';
        this.settingDataService.getSettingById(this.id)
            .subscribe(
                setting => {
  ...
  public onSubmit() {
    ...
    this.setting.meta_value += '';
    if (this.mode == 'create') {
      this.settingDataService.addSetting(this.setting)
          .subscribe(
              result => {
                if (result.success) {
      ...
      this.settingDataService.updateSettingById(this.setting)
          .subscribe(
              result => {
                if (result.success) {
                  this.settingDataService.refreshGlobalSettings();
