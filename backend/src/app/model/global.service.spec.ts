...
describe('global.service.ts', () => {
  ...
  beforeEach(() => {
    globalService = new GlobalService(new JwtHelperService());
  });
  ...
  describe('globalService.loadGlobalSettingsFromSessionStorage()', () => {
    it('should not set setting if backend-setting is not available in session storage', () => {
      sessionStorage.removeItem('backend-setting');
      globalService.loadGlobalSettingsFromSessionStorage();
      expect(globalService.setting).toEqual({});
    });

    it('should set setting if backend-setting is not available in session storage', () => {
      const tmpValue: any = {
        setting1: 'temporary setting'
      };
      sessionStorage.setItem('backend-setting', JSON.stringify(tmpValue));
      globalService.loadGlobalSettingsFromSessionStorage();
